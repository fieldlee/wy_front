              ...
              <v-row>
                ...
                <v-col cols="12" md="6">
                  <div class="text-center">
                    ...
                    <v-text-field color="secondary" label="用户名..." v-model="account" :rules="usernameValidation"
                    ...
                    <v-text-field label="密码..." :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
                      :rules="passwordValidation" :type="show ? 'text' : 'password'" color="secondary" v-model="password"
                      prepend-icon="mdi-lock-outline" @click:append.prevent="show = !show" />
                    <v-text-field label="密码..." :append-icon="show2 ? 'mdi-eye' : 'mdi-eye-off'"
                      :rules="passwordValidation" :type="show2 ? 'text' : 'password'" color="secondary"
                      v-model="repassword" prepend-icon="mdi-lock-outline" @click:append.prevent="show2 = !show2" />
                    <v-row>
                      <v-col cols="8">
...
import { getCaptcha, reqCheckkeys, reqRegister } from '../../utils/api'
export default {
  name: 'PagesRegister',
  ...
  methods: {
    ...
    register: function () {
      ...
      reqCheckkeys(check_obj).then((response) => {
        if (response.data.account == true) {
          this.alertErr(true, "您的账号已存在，请修改");
          return
        }
        //注册账号
        reqRegister(register).then((response) => {
          console.log(response);
          if (response.code == 0) {
